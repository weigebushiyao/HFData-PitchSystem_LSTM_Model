...
class ShowAndSave:
    ...
    def show_save_figure(self, detal_idx=10):
        ...
        if os.path.exists(
                self.fig_path + self.model_folder_name + '_' + self.model_kind + '_' + self.data_kind + '.png'):
            plt.savefig(
                self.fig_path + self.model_folder_name + '_' + self.model_kind + '_' + self.data_kind + '_' + self.time_now)
        else:
    ...
    def save_result(self, true_mean=None, pred_mean=None, train_n=None, test_n=None, hyper_params=None):
        ...
        rsqu = self.r_square()
        data = {'true_mean': true_mean, 'pred_mean': pred_mean, 'mse': mse, 'rmse': rmse, 'mae': mae, 'mape': mape,
                'R Square': rsqu, 'params': self.params, 'train_number': train_n, 'test_number': test_n,
                'hyper_params': hyper_params}
        print(data)
        df = pd.DataFrame(list(data.items()))
        # result_path=self.result_path +self.model_folder_name+'_'+self.model_kind+'_'+self.data_kind+ '_result.csv'
        if os.path.exists(
                self.result_path + self.model_folder_name + '_' + self.model_kind + '_' + self.data_kind + '_result.csv'):
            df.to_csv(
                self.result_path + self.model_folder_name + '_' + self.model_kind + '_' + self.data_kind + '_' + self.time_now + '_result.csv',
                encoding='utf-8')
        else:
            df.to_csv(
                self.result_path + self.model_folder_name + '_' + self.model_kind + '_' + self.data_kind + '_result.csv',
                encoding='utf-8', index=None, header=None)
    ...
    def save_result_dataframe(self, true, pred):
        ...
        if os.path.exists(
                self.true_pred_path + self.model_folder_name + '_' + self.model_kind + '_actual_prediction.csv'):
            df.to_csv(
                self.true_pred_path + self.model_folder_name + '_' + self.model_kind + '_' + self.time_now + '_actual_prediction.csv',
                encoding='utf-8')
        else:
            df.to_csv(self.true_pred_path + self.model_folder_name + '_' + self.model_kind + '_actual_prediction.csv',
                      encoding='utf-8')
